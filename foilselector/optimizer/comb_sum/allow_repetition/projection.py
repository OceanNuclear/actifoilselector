...
def determine_next_vector_symmetric(bottom_matrix):
    ...
    num_missing_rows = (
        num_dimensions - corner_len
    )  # number of repetition needed to fill in the beginning of the vector.
    ...
    wedge_product_vectors = stick_column_vector_onto_left_of_matrix(
        extra_left_column, corner
    )[::-1]
...
def determine_next_vector_linear(bottom_matrix):
    ...
    wedge_product_vectors[0, 0] = (
        sum(linear_indices(num_missing_rows)) / k * bottom_matrix[-1, 0]
    )

    product_vector = -wedge_product(wedge_product_vectors)
    next_basis_vector[:num_missing_rows] = (
        linear_indices(num_missing_rows) * product_vector[0] / k
    )
    next_basis_vector[num_missing_rows:] = product_vector[1:]
...
def wedge_product(matrix_formed_by_row_vectors):
    ...
    for i in range(shape[1]):
        left_matrix, right_matrix = (
            matrix_formed_by_row_vectors[:, :i],
            matrix_formed_by_row_vectors[:, i + 1 :],
        )
...
def projection_matrix(N, *, method="asymmetric_sqrt"):
    ...
    iszero = np.isclose(
        projection, 0, rtol=0, atol=np.finfo(projection.dtype).resolution
    )
    projection[iszero] = 0
