...
class CombinationNode(object):
    ...
    def get_children_coordinates(self, right=True):
        if right:
            return [
                (
                    self.plot_coordinates[0] + (num - (self.occupancy.sum() + 1) / 2),
                    self.plot_coordinates[1] - 1,
                )
                for num in self.right_movable_numbers
            ]
        else:
            return [
                (
                ...
                for num in self.left_movable_numbers
            ]

    def get_children(self, right=True):
        children = []
        movable_numbers = (
            self.right_movable_numbers if right else self.left_movable_numbers
        )
        for num_moved, child_coord in zip(
            movable_numbers, self.get_children_coordinates(right)
        ):
...
CombinationGraphState = namedtuple(
    "CombinationGraphState",
    ["all_nodes", "sorted_sum_result", "expectant_parents"],
)


def top_n_sums_generator(
    sorted_list, target_chosen_length: int, skip_n: int, largest=True
):
    """
    ...
    if largest:
        ...
        first_occupancy[-target_chosen_length:] = True
    children = [
        CombinationSumNode(first_occupancy, (0, 0), sorted_list),
    ]  # NULL births only 1 child: the origin (the first node).

    n_max = fac(len(sorted_list)) / (
        fac(len(sorted_list) - target_chosen_length) * fac(target_chosen_length)
    )
    ...
    while len(sorted_sum_result) < n_max:
        ...
        if len(sorted_sum_result) >= skip_n:
            yield CombinationGraphState(
                all_nodes.copy(), sorted_sum_result.copy(), expectant_parents.copy()
            )


def top_n_sums(sorted_list, target_chosen_length, n, largest=True):
    return next(
        top_n_sums_generator(sorted_list, target_chosen_length, n, largest)
    ).sorted_sum_result


def top_n_sums_of_dict(un_sorted_dict, target_chosen_length, n, largest=True):
    descending_sorted_dict = OrderedDict(
        sorted(un_sorted_dict.items(), key=itemgetter(1), reverse=True)
    )
    top_n_combos = top_n_sums(
        list(descending_sorted_dict.values()), target_chosen_length, n, largest
    )
