def plot_nodes(
    ...
    sorted_list, target_chosen_length = (
        sample_node.sorted_list,
        sample_node.occupancy.sum(),
    )
    biggest_sum, smallest_sum = (
        sorted_list[:target_chosen_length].sum(),
        sorted_list[-target_chosen_length:].sum(),
    )
    ...
    ax.scatter(
        *b_node,
        s=normalize_size(ary(birthed_sum)),
        marker="o",
        facecolors="none",
        edgecolors=primary_color,
    )  # nodes instantiated but not sorted.

    # draw the nodes
    b_edgex, b_edgey = (
        transform_matrix @ (ary(birthed_children_links) + offset).T
    ).reshape([2, -1, 2])
    ...
    if unbirthed:
        ...
        ax.scatter(
            *u_node,
            s=normalize_size(ary(unbirthed_sum)),
            marker="o",
            facecolors="none",
            edgecolors="C0",
        )  # nodes on the leading edge, instantiated but not sorted, whose children has not yet been instantiated
        if plot_dangling_links:
            u_edgex, u_edgey = (
                transform_matrix @ (ary(unbirthed_children_links) + offset).T
            ).reshape([2, -1, 2])
            u_lines = ax.plot(
                u_edgex.T, u_edgey.T, linestyle="--", color="C0"
            )  # link to children which aren't instantiated yet
...
def main(list_length, n_chosen, artisitc_plot=False, _MAKE_ANIMATION=False):
    ...
    if _MAKE_ANIMATION:
        ...
        with writer.saving(fig, vid_save_name, 300):
            for n, (down_dict, up_dict) in enumerate(
                zip(
                    top_n_sums_generator(sorted_list, n_chosen, 0, True),
                    top_n_sums_generator(sorted_list, n_chosen, 0, False),
                )
            ):
                if _ARTISTIC_PLOT:
                ...
                ax.set_ylim(
                    (ary([0, graph_depth]) * 1.05) - graph_depth * 0.05 / 2 - graph_depth
                )
                ...
                if _ARTISTIC_PLOT:
                    ax = plot_nodes(
                        *up_dict,
                        largest=False,
                        ax=ax,
                        offset=(+0, -graph_depth),
                        transform_matrix=ary([[-1, 0], [0, 1]]),
                        text=not _ARTISTIC_PLOT,
                    )
        ...
        for down_dict, up_dict in list(
            ...
            plt.savefig(
                "combination_graph/{}_choose_{}.png".format(list_length, n_chosen)
            )
            plt.show()
            ax.cla()
