...
def find_new_vec(ref_vector, *perp_vectors):
    ...
    coef_builder = [
        ref_vector,
        *perp_vectors,
    ]  # = basis to express our desired unit_vector in.
    first_n_minus_1_lines = [
        [basis_vec.dot(vec) for basis_vec in coef_builder] for vec in perp_vectors
    ]
    ...
    return NotImplementedError(
        "Unfortunately we need sympy to solve it, so this visualization will be a project that takes more time than I have."
    )
...
if __name__ == "__main__":
    ...
    new_coordinates = [
        sqrt(1 / num_dim) * np.ones(num_dim),
    ]
    while len(new_coordinates) < num_dim:
        reference_vec = (
            prev_coordinates.pop()
        )  # must be the projection of this vector on the remaining hyperplane space.
...
if False:
    ...
    writer = manimation.writers["ffmpeg"](
        fps=15, metadata={"title": "Video_name", "comment": "#", "artist": "Ocean Wong"}
    )
