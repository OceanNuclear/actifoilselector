...
class DecayPhoton(object):
    ...
    def __str__(self):
        return "< {} of {} eV from decay of {}>".format(
            self.__class__.__name__, self.energy, self.parent
        )
...
class GammaSpectrumABC(object):
    ...
    def list_peaks(self, sort_by=None, isotope_placeholder=""):
        ...
        output_list = [
            LineTuple(line.energy, line.intensity, isotope_placeholder, line)
            for line in self.lines
        ]
...
class SingleDecayGammaSignature(GammaSpectrumABC):
    ...
    def __init__(self, spectrum, isotope):
        ...
        for rad_type, rad_dict in spectrum.items():
            if rad_type == "xray":
                self.lines.extend([
                    XRayLine(rad, isotope) * rad_dict["discrete_normalization"]
                    for rad in rad_dict["discrete"]
                ])
            elif rad_type == "gamma":
                self.lines.extend([
                    GammaLine(rad, isotope) * rad_dict["discrete_normalization"]
                    for rad in rad_dict["discrete"]
                ])
    ...
    def __add__(self, another_signature):
        ...
        new_lines = [
            line1 + line2 for line1, line2 in zip(self.lines, another_signature.lines)
        ]
        return GammaSignature(new_lines, self.isotope)
    ...
    def __init__(self, line_collection, isotope):
        ...
        for line in line_collection:  # typechecking.
            if isinstance(line, DecayPhoton):
                ...
                raise TypeError(
                    "Must provide a list of DecayPhotons (Gamma or Xray) with properly scaled intensities as the argument to line_collections."
                )
...
class LineTuple(namedtuple("SortingLine", line_tuple_attributes)):
    def __str__(self):
        str_attr = [
            str(getattr(self, attr)).rjust(28)[:28] for attr in line_tuple_attributes[:2]
        ]
...
class GammaSpectrum(GammaSignature):
    ...
    def __add__(self, new_signature):
        ...
        for new_sig in new_signature:
            if new_sig.isotope in [old_sig.isotope for old_sig in old_signatures]:
                ...
                old_sig_index = [
                    old_sig_index
                    for old_sig_index, old_sig in enumerate(old_signatures)
                    if old_sig.isotope == new_sig.isotope
                ][0]
    ...
    def plot(self, ax=None, default_width=5, sqrt_scale=False):
        ...
        for sig in self.signatures:
            line_handles.append(
                sig.plot(ax, default_width, sqrt_scale, label=str(sig.isotope))[1]
            )
            # sig.plot() returns (ax, line); so sig.plot()[1] gives the line.
        return ax, line_handles

    def __str__(self):
        return "< {} consists of {} signatures >".format(
            self.__class__.__name__, len(self.signatures)
        )
    ...
    def sort_signatures_by_intensity(self):
        ...
        while len(self.signatures) > 0 and len(origins) > 0:
            ...
            indices = [
                ind
                for ind, sig in enumerate(self.signatures)
                if sig.isotope == target_iso
            ]
    ...
    def merge_into_amophorus_spectrum(self):
        energy, intensity, source_isotope, _photon_type = ary(
            self.list_peaks(sort_by_intensity=None)
        ).T.reshape([4, -1])
        ...
        for E in sorted(set(energy)):
            ...
            new_photon_dict = {
                ...
                "origin": {
                    ori: intense
                    for ori, intense in zip(source_isotope[mask], intensity[mask])
                },
            }
            new_lines.append(DecayPhoton(new_photon_dict))
        return MergedGammaSpectrum(
            new_lines, ordered_set(source_isotope)
        )  # ordered_set is a list
...
class MergedGammaSpectrum(GammaSpectrum):
    ...
    def __init__(self, spectrum, isotope_list):
        ...
        for isotope in isotope_list:
            ...
            selected_spectrum = [
                line for line in spectrum if isotope in line.origin.keys()
            ]
            ...
            matching_indices = [
                ind
                for ind, sig_old in enumerate(self.signatures)
                if sig_old.isotope == last_iso
            ]
