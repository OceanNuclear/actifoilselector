...
def save_atomic_composition_json(
    processed_composition, json_path=".atomic_composition.json"
):
    from os import path
...
def save_decay_radiation(
    decay_radiation, json_path=".decay_radiation.json", verbose=True
):
    ...
    with open(json_path, "w") as j:
        if verbose:
            print("Writing to decay_radiation.json... ", end="")
...
def save_decay_info(decay_info, json_path=".decay_info.json"):
    with open(json_path, "w") as j:
        return json.dump(
            decay_info, j, cls=EncoderOpenMC
        )  # we're dumping the condensed decay dict, i.e. called the decay_info, here.
...
def read_counts_csv(csv_path=".counts.csv", return_parameters_stored_in_comments=False):
    ...
    if return_parameters_stored_in_comments:
        ...
        return unserialize_pd_DataFrame(
            pd.read_csv(csv_path, index_col=[0], comment="#")
        ), parameters
    else:
        return unserialize_pd_DataFrame(
            pd.read_csv(csv_path, index_col=[0], comment="#")
...
def read_vector(file_path):
    ...
    with open(file_path) as f:
        scalar_list = re.split(
            "\n|,| ", f.read()
        )  # with possible empty entries if there are blank lines.
