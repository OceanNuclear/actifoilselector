...
def HPGe_efficiency_curve_generator(file_location=default_file_path, deg=4, cov=True):
    ...
    if "MeV" in datapoints.columns[0] or "MeV" in file_location:
        E = MeV * E
    print(
        "Using the gamma detection efficiency curve read from {}".format(file_location)
    )
    if datapoints.values.T[2:].shape[0] > 0:  # assume that column is the error
        sigma = datapoints.values.T[2]
        print(
            "Using the 3rd column of ",
            file_location,
            r" as error on the efficiency measurements (\sigma)",
        )
        cov = "unscaled"
        w = 1 / sigma**2
    else:
    ...
    p = polyfit(
        ln(E), ln(eff), deg, w=w, cov=cov
    )  # make sure that the curve points downwards at high energy
    if cov:
        p, pcov = p
    ...
    if not p[0] < 0:
        ...
        for i in range(mindeg, maxdeg + 1):
            ...
            if p[0] < 0:
                print(
                    "a {0} order polynomial fit to the log(energy)-log(efficiency) curve is used instead of the default {1} order".format(
                        i, deg
                    )
                )
                break
                ...
                i == maxdeg
            ):  # if we've reached the max degree tested and still haven't broken the loop, it means none of them fits.
                print(
                    "None of the polynomial fit in log-log space is found to extrapolate properly! using the default {0} order fit...".format(
                        deg
                    )
                )
    ...
    def efficiency_curve(E):
        if isinstance(E, unc.core.AffineScalarFunc):
            ...
            lnE = ln(E)
        lneff = np.sum(
            [p[::-1][i] * lnE**i for i in range(len(p))], axis=0
        )  # coefficient_i * x ** i
        ...
        if cov:
            ...
            if isinstance(E, unc.core.AffineScalarFunc):
                ...
                variance_from_E = (
                    sum([p[::-1][i] * i * lnE ** (i - 1) for i in range(1, len(p))]) ** 2
                    * (error_of_lnE) ** 2
                )
