from openmc.data import AVOGADRO
...
def mass_to_num_atoms(mass_dict, fraction_dict):
    ...
    for foil_name, foil_mass_grams in mass_dict.items():
        ...
        avg_atom_mass_amu = sum([
            atomic_mass(isotope) * abundance for isotope, abundance in fractions.items()
        ])
        ...
        num_atoms[foil_name] = {
            isotope: total_num_atoms * abundance
            for isotope, abundance in fractions.items()
        }
    return num_atoms
...
def get_fraction(composition):
    ...
    numerical_value = "".join([
        i for i in composition[1:] if (i.isnumeric() or i == ".")
    ])
...
def expand_bracket(solid):
    ...
    if len(left_bracket_split := solid.split("(")) > 1:
        ...
        for char in behind_brackets:  # we will be looping through a COPY of the behind_brackets, so we won't have to worry about mutating the string while looping.
            ...
            behind_brackets = behind_brackets[
                1:
            ]  # that character is considered "Used", and is removed from the behind_brackets variable.
...
def get_natural_average_atomic_mass(elem, verbose=False, **kwargs):
    ...
    **kwargs is plugged into the numpy.isclose function ot modify rtol and atol.
    ...
    isotopic_abundance = {
        ...
        if extract_elem_from_string(isotope) == elem
    }
    if sum(isotopic_abundance.values()) != 1.0:
        if verbose:
            print(
                "Expected the isotopic abundances of {} to sum to 1.0, got {} instead.".format(
                    elem, sum(isotopic_abundance.values())
                )
            )
        if not np.isclose(sum(isotopic_abundance.values()), 1.0, **kwargs):
            raise ValueError(
                "The sum of abundances for element '{}' = {} is not close to unity.".format(
                    elem, sum(isotopic_abundance.values())
                )
            )
    return sum([
        atomic_mass(isotope) * abundance
        for isotope, abundance in isotopic_abundance.items()
    ])
...
def get_elemental_fractions(formula):
    if ("/" not in formula) or ("(Atomic%)" in formula):
        ...
        compositions = split_at_cap(
            formula.replace("(Atomic%)", "")
        )  # could've used removesuffix in python 3.9; but I've not installed that yet.
        elements = [extract_elem_from_string(comp) for comp in compositions]

        raw_mole_ratios = [
            get_fraction(comp) for comp in compositions
        ]  # interpret them as mole ratios

        elemental_fractions = {
            elem: mole_ratio / sum(raw_mole_ratios)
            for mole_ratio, elem in zip(raw_mole_ratios, elements)
        }
    else:
        ...
        weight_fractions = [
            get_fraction(comp) / 100.0 for comp in compositions
        ]  # interpret them as weight fraction
        average_mass = 1 / sum([
            weight / get_natural_average_atomic_mass(elem)
            for weight, elem in zip(weight_fractions, elements)
        ])  # obtain the average mass which is then used as a multiplier below:

        elemental_fractions = {
            elem: weight / get_natural_average_atomic_mass(elem) * average_mass
            for weight, elem in zip(weight_fractions, elements)
        }
...
def convert_elemental_to_isotopic_fractions(dict_of_elemental_fractions):
    ...
    for elem, fraction in dict_of_elemental_fractions.items():
        matching_isotopes = [
            iso
            for iso in NATURAL_ABUNDANCE.keys()
            if extract_elem_from_string(iso) == elem
        ]
...
def get_average_atomic_mass_from_isotopic_fractions(dict_of_isotopic_fractions):
    return sum([
        atomic_mass(isotope) * fraction
        for isotope, fraction in dict_of_isotopic_fractions.items()
    ])


def get_average_atomic_mass_from_elemental_fractions(dict_of_elemental_fractions):
    return sum([
        get_natural_average_atomic_mass(elem) * fraction
        for elem, fraction in dict_of_elemental_fractions.items()
    ])
...
def pick_material(species, expanded_physical_prop_df):
    ...
    if expanded_physical_prop_df[species][material] > 0:
        ...
        dummy_data_series = pd.Series({
            col: float("nan") for col in expanded_physical_prop_df.columns
        })
