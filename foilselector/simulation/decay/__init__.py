    ...
    for subchain in linearize_decay_chain(
        ...
        new_pathway = {
            ...
            "counts during measurement": mat_exp_num_decays(
                subchain.branching_ratios, subchain.decay_constants, a, b, c
            )
            * subchain.countable_photons,
            "counts in 1 hr immediately after measurement": mat_exp_num_decays(
                subchain.branching_ratios, subchain.decay_constants, a, c, c + 3600
            )
            * subchain.countable_photons,
        }
        pre_population_of_this_pathway = mat_exp_population_convolved(
            subchain.branching_ratios, subchain.decay_constants, a, b
        )
        post_population_of_this_pathway = mat_exp_population_convolved(
            subchain.branching_ratios, subchain.decay_constants, a, c
        )
        ...
        new_pathway["decay rate pre-measurement"] = (
            subchain.decay_constants[-1] * pre_population_of_this_pathway
        )
        new_pathway["decay rate post-measurement"] = (
            subchain.decay_constants[-1] * post_population_of_this_pathway
        )
        new_pathway["count rate pre-measurement"] = (
            subchain.countable_photons * new_pathway["decay rate pre-measurement"]
        )
        new_pathway["count rate post-measurement"] = (
            subchain.countable_photons * new_pathway["decay rate post-measurement"]
        )
...
Summed_reaction_rates_results = namedtuple(
    "Summed_reaction_rates_results",
    [
        "total_counts",
...
def summed_cnt_and_rates(decay_info_dict: dict, root_product, a, b, c):
    ...
    total_counts_in_the_1h_following_measurement = sum([
    ...
    pre_measurement_count_rate = sum([
        path["count rate pre-measurement"]
        for path in detected_counts_per_primary_product
    ])
    post_measurement_count_rate = sum([
