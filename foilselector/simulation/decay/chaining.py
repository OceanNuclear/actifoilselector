...
class IsotopeDecay(
    namedtuple(
        "IsotopeDecayTuple",
        ["names", "branching_ratios", "decay_constants", "countable_photons"],
    )
):
    """
    ...
    def __add__(self, subordinate):
        return IsotopeDecay(
            self.names + subordinate.names,
            self.branching_ratios + subordinate.branching_ratios,
            self.decay_constants + subordinate.decay_constants,
            subordinate.countable_photons,
        )
...
def build_decay_chain_tree(decay_parent, decay_dict, decay_constant_threshold=1e-23):
    ...
    if decay_parent not in decay_dict:
        if "_" in decay_parent:
            ...
            return build_decay_chain_tree(
                decay_parent.split("_")[0], decay_dict, decay_constant_threshold
            )
        else:
    ...
    if decay_dict[decay_parent]["decay_constant"] <= decay_constant_threshold:
        ...
        for name, branching_ratio in decay_dict[decay_parent]["branching_ratio"].items():
            if name != decay_parent:
                tree["modes"].append({
                    "daughter": build_decay_chain_tree(name, decay_dict),
    ...
    if "modes" in decay_file:  # expand the decay modes if there are any.
        for mode in decay_file["modes"]:
            ...
            for subbranch in this_branch:
                subbranch.branching_ratios[0] = mode[
                    "branching_ratio"
                ]  # only a 'mode["branching_ratio"]' number of them goes into this subbranch.
...
def trim_tree(tree):
    ...
    if "modes" in tree.keys():
        results.update({
            " L=>": [
                (mode["branching_ratio"], trim_tree(mode["daughter"]))
                for mode in tree["modes"]
            ]
        })
    return results
