...
def vogels_model(num_points):
    ...
    angle = (
        np.arange(num_points) * tau * (1 - golden)
    )  # I like using the negative version of the golden ratio more.
...
def assert_isclose_mean(*args, varname="parameter"):
    ...
    *args : Each of which is a number, or a numpy array of identical shapes.
    ...
    if len(args) > 2:
        return np.mean(
            [
                assert_isclose_mean(a, b, varname=varname)
                for a, b in zip(args, np.roll(args, 1, axis=0))
            ],
            axis=0,
        )
    elif len(args) == 2:
        ...
        if isinstance(comparison, bool):
            ...
            assert comparison.all(), (
                varname + " calculated in two different ways must match!"
            )
...
class Disk:
    ...
    def __init__(self, points):
        ...
        if len(points) == 1:
            ...
            c1 = (
                midpoints[0]
                + assert_isclose_mean(l1, _l1, varname="lambda 1") * bisector_dir[0]
            )
            c2 = (
                midpoints[1]
                + assert_isclose_mean(l2, _l2, varname="lambda 2") * bisector_dir[1]
            )
            c3 = (
                midpoints[2]
                + assert_isclose_mean(l3, _l3, varname="lambda 3") * bisector_dir[2]
            )
            self.center = assert_isclose_mean(c1, c2, c3, varname="center point of disk")
            self.radius = assert_isclose_mean(
                *[np.linalg.norm(self.center - p) for p in points], varname="radius"
            )
            # vectorize the above.
            # and then find the stopping point that these parameter (l)'s represents
        else:
            raise ValueError(
                f"{len(points)} points is an invalid number of points for uniquely defining a circle!"
            )
    ...
    def plot(self, ax):
        ...
        ax.plot(
            self.center[0] + self.radius * np.cos(line),
            self.center[1] + self.radius * np.sin(line),
        )
