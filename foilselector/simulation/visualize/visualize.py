...
reactant, direct_product, indirect_product = (
    "reactant",
    "direct product",
    "indirect product",
)
...
def harvest_multiple_trees(primary_products, decay_dict):
    ...
    """
    leaf_sets = [
        leaf_collector(build_decay_chain_tree(iso, decay_dict))
        for iso in primary_products
    ]
...
def as_graph(num_reactions, selected_decay_dict):
    ...
    for parent, decay_info in selected_decay_dict.items():
        try:
            half_life[parent] = (
                0.6931471805599453 / decay_info["decay_constant"]
            )  # log 2 divided by that
    ...
    pos = nx.spring_layout(
        dg,
        pos={**reactant_pos, **other_pos},
        fixed=list(reactant_pos.keys()),
        weight=None,
        iterations=200,
...
def shell_layout(graph, init_pos, finer_division=20):
    ...
    isotope_type = {
        ...
        for iso_type in [
            ...
            "Judas",
            direct_product,
            direct_product + indirect_product,
            indirect_product,
        ]
    }
    ...
    for node, data in graph.nodes(data=True):
        if reactant in data[identity]:
            ...
            elif (direct_product in data[identity]) and (
                indirect_product not in data[identity]
            ):
                isotope_type[direct_product].append(node)
            elif (direct_product in data[identity]) and (
                indirect_product in data[identity]
            ):
                isotope_type[direct_product + indirect_product].append(node)
            elif (direct_product not in data[identity]) and (
                indirect_product in data[identity]
            ):
                isotope_type[indirect_product].append(node)
            else:
                raise ValueError(
                    f"Missed this case which has node.{identity}=\n{data[identity]}\n(Poor programming on my part!)"
                )

    segmented_shells = [
        [{} for j in range(finer_division)] for shell_key in isotope_type.keys()
    ]
    ...
    for i, (shell_key, isotopes) in enumerate(isotope_type.items()):
        ...
        for iso in isotopes:
            ...
            phase = (
                phase + (tau) if phase < 0 else phase
            )  # turn the negative angles into positive.
            init_angles[iso] = phase
        init_angles = dict(
            sorted(init_angles.items(), key=lambda x: x[1])
        )  # sort it so it is ordered in anti-clockwise.

        nodes, angle_values = (
            ary(list(init_angles.items()), dtype="O").reshape([-1, 2]).T
        )
        ...
        for j in range(finer_division):
            ...
            segmented_shells[i][j] = dict(
                zip(nodes[in_this_segment], angle_values[in_this_segment])
            )
    ...
    def sort_to_nearestbin(dict_of_positions, bin_centroids):
        ...
        unassigned, empty = (
            np.ones(scalars.shape, dtype=bool),
            np.ones(bins.shape, dtype=bool),
        )
        ...
        while any(unassigned):
            distance_to_bins = abs(
                np.subtract.outer(scalars[unassigned], bin_centroids[empty])
            )
            nearest_match_ind = np.unravel_index(
                distance_to_bins.argmin(), distance_to_bins.shape
            )
        ...
        for j in range(finer_division):
            ...
            for this_angle, occupied in zip(bin_comb + angle_offset, occupancy_vector):
                if occupied:
                    ...
                    filler_counter += (
                        1  # prevent name collision and merging of filler points.
                    )
...
def position_initializer(reactants, other_isotopes, seed=None):
    ...
    fdisk = Disk(
        fermat_spiral[-3:]
    )  # define the boudning circle, using only the last 3 points of the fermat spiral.
    fermat_spiral_area = np.pi * (fdisk.radius**2)
    if fermat_spiral_area == 0:
        fermat_spiral_area = (
            np.pi
        )  # if there is only 1 point, then we force the area to be non-zero.

    # shift and shrink down the spiral so that it matches the point density.
    node_density = (
        len(reactants) + len(other_isotopes)
    ) / 4  # the [-1, 1], [-1, 1] square has area =4.
    scale_factor = (
        len(reactants) / node_density / fermat_spiral_area
    ) ** 0.5  # need to scale down by this much.
    ...
    exclusion_disk = Disk.from_parameters(
        [0.0, 0.0], fdisk.radius * scale_factor * 1 + EXCLUSION_MARGIN
    )  # hard-coded value of 15% more. Can change later.
...
def draw_networkx_weighted_directed_graph(
    graph,
    ax,
    minarrow=0.1,
    maxarrow=1,
    cmap=default_cmap,
    write_half_life=False,
    edge_labels=False,
):
    """
    ...
    nx.draw_networkx_nodes(
        graph, pos, node_color=[cmap(attr) for n, attr in graph.nodes(data=True)], ax=ax
    )
    if write_half_life:
    ...
    if len(graph.edges) > 0:
        ...
        for weight in unique_weights:
            ...
            weighted_edges = [
                (n1, n2)
                for (n1, n2, edge_attr) in graph.edges(data=True)
    ...
    if edge_labels:
        if isinstance(edge_labels, bool):
            ...
            raise ValueError(
                "Unrecognized edge_labels! Can only use True (in which case node.str_weight will be used), False/None, or a dictionary to represent what labels to use."
            )
...
def irradiationView(graph):
    ...
    subgraph_list = [
        node
        for node, dat in graph.nodes(data=True)
        if (reactant in dat[identity]) or (direct_product in dat[identity])
    ]
...
def decayView(graph, exclude_further_decay=False):
    ...
    subgraph_list = [
        node
        for node, dat in graph.nodes(data=True)
        if (direct_product in dat[identity]) or (indirect_product in dat[identity])
    ]
    subgraph = graph.subgraph(subgraph_list)
    ignore_edges = []
    for innode, outnode in subgraph.edges:
