...
def calculate_num_decays_measured_per_product_present(
    root_product, schedule, decay_info, time_resolved=False
):
    """
    ...
    for subchain in linearize_decay_chain(decay_tree):
        ...
        num_decays_during_measurement = sum(
            ...
            * step.fluence
            / total_fluence  # normalized to 1 unit fluence
...
def merge_same_isotope_populations(dict_of_df, sort_key="number of photons measurable"):
    ...
    if sort_key:
        final_isotope_population_df.sort_values(
            sort_key, ascending=False, inplace=True
        )  # sort descendingly
