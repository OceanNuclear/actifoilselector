...
end_step_kw = (
    ...
    "SPECTRUM",
    "ATOMS",
)  # keyword to indicate that irradiation has ended for this phase
...
class Schedule:
    ...
    def __init__(self, *step_list):
        ...
        self.is_gamma_measurement = [
            isinstance(step, GammaSpectrometryStep) for step in step_list
        ]
    ...
    while len(keywords_and_params) > 0:
        ...
        if keyword.upper() == "FLUX":
            ...
            try:
                ...
                duration_seconds = (
                    numerical_duration * unit_conversion[unit]
                )  # raises KeyError if next_word_probably_unit isn't a time unit, and skips the following line.
            ...
            keyword.upper() == "ZERO"
        ):  # only exist for fispact compatibility, I suppose.
...
def cut_text_at_sample(full_control_text):
    ...
    """
    material_names, material_schedule_text = (
        [],
        [],
    )  # container for the material names and the main text describing the material irradiation schedules.
