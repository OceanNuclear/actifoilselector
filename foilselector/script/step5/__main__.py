...
def main(dirname):
    ...
    with open(os.path.join(dirname, "irradiation_schedule.txt")) as f:
        schedule_of_materials, efficiency_curve_dict = read_fispact_irradiation_schedule(
            f.read()
        )
    ...
    def get_macroscopic_xs_all_rx(isotope_number_densities):
        ...
        for isotope_name, density in isotope_number_densities.items():
            ...
            for reaction_name, xs_profile in matching_parent.iterrows():
                ...
                macroscopic_xs[product] += xs_profile.values * (
                    BARN * density
                )  # accumulate cross-section
    ...
    def calculate_num_decays_measured_per_product_present(root_product, schedule):
        ...
        for subchain in linearize_decay_chain(decay_tree):
            ...
            if NO_TIME_RESOLUION:
                ...
                isotope_name = arrow.join(
                    subchain.names
                )  # give the full pathway where the isotope came from.
            collected_num_decays[isotope_name] += num_decays_during_measurement
            collected_num_photons[isotope_name] += (
                num_decays_during_measurement * subchain.countable_photons
            )
        ...
        return pd.DataFrame({
            "number of decays": num_decays_series,
        ...
        for root_isotope, df in dict_items[1:]:
            for final_isotope, populations in df.iterrows():
                if final_isotope not in final_isotope_population_df.index:
                    final_isotope_population_df = final_isotope_population_df.append(
                        populations
                    )
                else:
                    final_isotope_population_df.loc[final_isotope] += populations
        if sort:
            final_isotope_population_df.sort_values(
                "number of photons measurable", ascending=False, inplace=True
            )  # sort descendingly
    ...
    for foil_name, foil in selected_foils.items():
        print(
            "for foil =",
            foil_name,
            "a schedule is detected as the following:",
            schedule_of_materials[foil_name],
        )

        thickness = foil["thickness (cm)"]
        area = foil[
            "area (cm^2)"
        ]  # work in cms units, so the flux (which will cancel this out) will also be in cm2 unit
        ...
        num_reaction_per_unit_fluence = pd.Series(
            (-expm1(-macroscopic_xs_df * thickness)).values
            @ neutron_energy_pdf_apriori
            * area,
        ...
        num_decays_sorted_by_root_product[foil_name] = {
            root_product: num_reactions
            * calculate_num_decays_measured_per_product_present(
    ...
    for foil_name, reaction_products in num_decays_sorted_by_root_product.items():
        print(
            f"Summing together the population of isotopes obtained from different pathways in {foil_name}..."
        )
        summed_isotope_populations[foil_name] = merge_same_isotope_populations(
            reaction_products
        )
        num_decays_spec[foil_name] = GammaSpectrum()

        for final_isotope, populations in summed_isotope_populations[
            foil_name
        ].iterrows():
            spectrum = decay_dict[final_isotope.split(arrow)[-1]].get("spectra", {})
            num_decays_spec[foil_name] += (
                SingleDecayGammaSignature(spectrum, final_isotope)
                * populations["number of decays"]
            )
        num_detected_spec[foil_name] = (
            num_decays_spec[foil_name] * efficiency_curve_dict[foil_name]
        )
...
def main_old(dirname):
    ...
    def get_macro_and_num_reactions(parents_product_mts, foil_num_density_dict):
        ...
        for parent_product_mt in unpack_reactions(parents_product_mts):
            microscopic_xs = (
                sigma_df[sigma_df.index == parent_product_mt].values[0] * BARN
            )
            macroscopic_xs += (
                microscopic_xs * foil_num_density_dict[parent_product_mt.split("-")[0]]
            )
    ...
    for foil_name, foil in selected_foils.items():
        ...
        total_counts_dict = foil[
            [k for k in foil.keys() if k.startswith("total counts at thickness =")][0]
        ]
        ...
        for parents_product_mts in total_counts_dict.keys():
            ...
            macroscopic_xs, num_reactions = get_macro_and_num_reactions(
                parents_product_mts, foil["number density (cm^-3)"]
            )
            if PLOT_MACRO:
                ax = plt.subplot()
                ax.semilogx(
                    gs.flatten(), np.repeat(macroscopic_xs / (macroscopic_xs).sum(), 2)
                )
            ...
            if PLOT_CONTR:
                ax = plt.subplot()
                ax.semilogx(
                    gs.flatten(), np.repeat(num_reactions / (num_reactions).max(), 2)
                )
            ...
            for subchain in linearize_decay_chain(decay_tree):
                num_decays_recorded_per_reaction = mat_exp_num_decays(
                    subchain.branching_ratios,
                    subchain.decay_constants,
                ...
                num_decays_recorded = (
                    num_reactions
                ).sum() * num_decays_recorded_per_reaction
                ...
                final_spec += (
                    SingleDecayGammaSignature(spectrum, subchain.names)
                    * num_decays_recorded
                )  # use the entire subchain as the isotope name

            if PLOT_INDIVIDUAL_SPEC:
                ax, lines = final_spec.plot()
                ax.set_title(
                    "Decay of {} in {}\n before multiplying by efficiency".format(
                        product, foil_name
                    )
                )
                plt.show()
                ...
                ax.set_title(
                    "Decay of {} in {}\n after multiplying by efficiency".format(
                        product, foil_name
                    )
                )
                plt.show()
