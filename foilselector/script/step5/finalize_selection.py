...
from foilselector.foldermanagement import get_apriori, get_parameters_json
...
from foilselector.decay import (
                        linearize_decay_chain,
                        build_decay_chain_tree,
                        decay_mat_exp_num_decays,
                        decay_mat_exp_population_convolved
                        )
from foilselector.reactionnaming import unpack_reactions
ln2 = np.log(2)
arrow = "->"
# grep for """P A T H W A Y   A N A L Y S I S   F O R   I R R A D I A T I O N   P H A S E""" in the fispact file and see if there are unexpected nuclide pathways
# And also check that their populations matches the numbers calculated by the FoilSelector
"""
Instruction:
This module is expected to be used after the following steps
- ```python RankFoils.py. [my_directory/]```
...
class GammaSpectrumABC(object):
    def _init_plot_values(self, default_width=5, unit="keV"):
        """
...
class SingleDecayGammaSignature(GammaSpectrumABC):
    ...
    def __init__(self, spectrum, isotope):
        ...
        Parameters
        ----------
        spectrum: dict accessed by e.g. 'openmc.data.Decay.from_file("decay/decay/O16")["spectra"]'
...
def fispact_FUEL_dict_string(num_reactants_dict):
    s = pprint.pformat(num_reactants_dict).replace("}"," ").replace("{", " ").replace("'", "").replace(": ", " ")
    s = "\n".join(line.strip().strip(",") for line in s.split("\n")) # trim line
    s = s.replace(", ", "\n")
    return f"FUEL {len(num_reactants_dict)}\n"+s
