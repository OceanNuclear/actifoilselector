...
def stage2_interpret_ap_energy(directory: Path, apriori: npt.NDArray, in_unit: str, *, group_or_point: str):
    ...
    if group_or_point=='group-wise':
        ...
        E_values = get_column_interactive(directory, 'energy paired to each data point of the a priori')
        ...
        if in_unit=='PUL':
            ...
            raise NotImplementedError("Point-wise (continuous) data should never be an integrated flux!")
...
def stage2_plot_apriori(E_values: npt.NDArray[float], apriori: npt.NDArray[float], continuous_apriori: Tabulated1D, apriori_gs: npt.NDArray):
...
def stage3_modify_apriori(E_values: npt.NDArray[float], apriori: npt.NDArray[float], continuous_apriori: Tabulated1D):
...
def stage4_add_uncertainty(directory: Path, apriori: npt.NDArray[float], continuous_apriori: Tabulated1D, apriori_copy: npt.NDArray[float], scheme: int, E_values: npt.NDArray[float])-> tuple[Tabulated1D, Tabulated1D] | None:
    section_title("4. [optional] adding an uncertainty to the a priori.")
    if ask_yn_question("Does the a priori spectrum comes with an associated error (y-error bars) on itself?"):
...
def stage5_load_group_structure(directory: Path, apriori_gs: npt.NDArray, E_values: npt.NDArray[float], continuous_apriori: Tabulated1D):
    section_title("5. Load in group structure.")
    if ask_yn_question("Should a different group structure than the apriori_gs (entered above) be used?"): # same gs as the a priori
        gs_source = ask_question("Would you like to read the gs_bounds 'from file' or manually create an 'evenly spaced' group structure?", ['from file', 'evenly spaced'])
...
def stage5_save_apriori_files(directory: Path, gs_array: npt.NDArray, continuous_apriori, error_present: tuple[Tabulated1D, Tabulated1D] | None):
...
def stage6_load_and_save_gamma_resolution(directory: Path):
    ...
    fwhm_examples = ";\n".join(f"FWHM = {default_res_func(peak)} keV at E={peak} keV" for peak in [511, 662, 1173, 1332])
    print(f"Default resolution is\n{fwhm_examples}.")
    if ask_yn_question("Would you like to provide your own resolution curve instead of using the default resolutions? (no = use default)"):
        if ask_yn_question("Do you have the coefficients in the resolution curve R(E)=√(x_0+x_1*E+x_2*E^2+...) (where E and R(E) have unit keV)? (y/n)"):
            while True:
                coef_str = input("Please enter the list of coefficients, separated by comma, in increasing degree of the coefficients: ")
                try:
            ...
            E_keV, full_path = get_column_interactive(
                directory, "Mean energy of the peaks (in keV)", output_full_file_path=True
            )
            ...
            while True:
                try:
                    degree_of_fit = ask_question("How many degrees of coefficient shall be fitted (i.e. how precise should the fitting polynomial be)? (Please enter number between [0-3])", "0123")
...
def stage7_load_and_save_gamma_peak_to_Compton_ratio(directory: Path):
    ...
    default_CS_func = Compton_to_peak_curve_factory(get_default_peak_to_Compton_coefficients())
    cs_ratio_examples = ";\n".join(f"P/C = {1/default_CS_func(peak)} keV at E={peak} keV" for peak in [511, 662, 1173, 1332])
    print(f"Default peak-to-Compton ratio is\n{cs_ratio_examples}.")
    if ask_yn_question("Would you like to provide your own peak-to-Compton curve instead of using the default photopeak-to-Compton ratios? (no = use default)"):
        if ask_yn_question("Do you have the coefficients for the peak-to-Compton ratios PC(E) = x_0+x_1*E+x_2*E^2+... (where E has unit keV)? (y/n)"):
            while True:
                coef_str = input("Please enter the list of coefficients, separated by comma, in increasing degree of the coefficients: ")
                try:
                    coefficients = ary([float(c) for c in coef_str.split(",")])
            ...
            pc = get_column_interactive(
                directory, "Peak-to-Compton ratio of the peaks (dimensionless)", file_path_given=full_path
            )
            while True:
                try:
                    degree_of_fit = ask_question("How many degree of coefficient shall be fitted (i.e. how precise should the fitting polynomial be)? (Please enter number between [0-2])", "012")
...
In the provided directory {}, the following .csv files are found:""".format(directory) )
    ...
    E_values, apriori, continuous_apriori, apriori_gs, scheme = stage2_interpret_ap_energy(
    ...
    E_values, apriori, continuous_apriori = stage3_modify_apriori(E_values, apriori, continuous_apriori)

    error_present = stage4_add_uncertainty(directory, apriori, continuous_apriori, orig_apriori_raw_values, scheme, E_values)

    gs_array = stage5_load_group_structure(directory, apriori_gs, E_values, continuous_apriori)
    ...
    peak_to_Compton_coefficients = stage7_load_and_save_gamma_peak_to_Compton_ratio(directory)
...
if __name__=="__main__":
    ...
    if len(sys.argv)==1:
        ...
        raise ValueError(f"Usage: `python {sys.argv[0]} [directory_to_act_on]`,\nwhere directory_to_act_on = '.' by default.")
