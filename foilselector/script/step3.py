...
def main(
    ...
    print(
        "Removing all reactions whose products are the same as the reactants (i.e. elastic scattering reactions):"
    )
    _trivial_rx = ary(sigma_df.index)[
        [
    ...
    population = pd.DataFrame(
        {
            "production of primary product per reactant atom": (sigma_df.values * BARN)
            @ fluence
        },
    ...
    cnt_rate_post = {}
    product_set = set([
        parent_product_mt.split("-")[1] for parent_product_mt in population.index
    ])
    ...
    for product in tqdm(
        ...
        total_counts_post_meas[product] = (
            count_calculation_results.total_counts_in_the_1h_following_measurement
        )
    ...
    population["final counts measured by detector PPP"] = ary([
    ...
    population.sort_values(
        "final counts measured by detector PPP", inplace=True, ascending=False
    )

    # shorthands.
    parent_product_mt_list = list(population.index)
    product_list = [
        parent_product_mt.split("-")[1] for parent_product_mt in parent_product_mt_list
    ]
    reactant_list = [
        parent_product_mt.split("-")[0] for parent_product_mt in parent_product_mt_list
    ]

    # add the rest of the information
    population["counts accumulated in the 1 hour following the detection period PPP"] = (
        ary([total_counts_post_meas[prod] for prod in product_list])
    )
    ...
    population["activity before measurement PPP"] = ary([
        activity_pre[prod] for prod in product_list
    ])
    population["activity after measurement PPP"] = ary([
        activity_post[prod] for prod in product_list
    ])
    population["detector count rate before measurement PPP"] = ary([
        cnt_rate_pre[prod] for prod in product_list
    ])
    population["detector count rate after measurement PPP"] = ary([
        cnt_rate_post[prod] for prod in product_list
    ])
    population["max microscopic cross-section"] = ary([
        ss_info[parent_product_mt] for parent_product_mt in parent_product_mt_list
    ])
    ...
    save_counts_csv(
        population,
        comments=[
            f" irradiation_duration = {irradiation_duration}",
    ...
    for foil_name, foil_comp in processed_composition.items():
        ...
        for component, fraction in foil_comp.items():
            ...
            _this_max_sigma = proportional_counts_xs[
                "max microscopic cross-section"
            ].max()
            max_sigma_so_far = max([
                max_sigma_so_far,
                _this_max_sigma,
            ])  # still working in BARN
            init_count_rate_per_reactant += (
                proportional_counts_xs["production of primary product per reactant atom"]
                * proportional_counts_xs["detector count rate before measurement PPP"]
            ).sum()
        ...
        max_num_atoms = max_num_atoms_from_count_rate_limit(
            nom(init_count_rate_per_reactant), max_gamma_count_rate
        )
        foil_dimension_limits[foil_name] = {
            "max. sigma(E) (energy-dependent cross-section) (barns)": max_sigma_so_far,
