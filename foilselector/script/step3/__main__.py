...
from foilselector.simulation.decay import decay_mat_exp_num_decays, Bateman_convolved_generator
...
PARAMS_DICT = get_parameters_json(directory)
IRRADIATION_DURATION = PARAMS_DICT["IRRADIATION_DURATION"]
TRANSIT_DURATION = PARAMS_DICT["TRANSIT_DURATION"]
MEASUREMENT_DURATION = PARAMS_DICT["MEASUREMENT_DURATION"]

apriori_flux, apriori_fluence = get_apriori(directory, IRRADIATION_DURATION)

with open(os.path.join(directory, CONDENSED_DECAY_INFO_FILE), 'r') as f:
    decay_dict = json.load(f)
    decay_dict = unserialize_dict(decay_dict)
sigma_df = pd.read_csv(os.path.join(directory, 'response.csv'), index_col=[0])
...
try:
    population.to_csv(os.path.join(directory, 'counts.csv'), index_label='rname')
...
save_parameters_as_json(directory, dict(
