...
parg = argparse.ArgumentParser(
    description="""Extract the relevant cross-sections and decay data from the nuclear data library.
...
.ecc (GENIE/ISOCS output) """,
    default=os.path.join(
        os.path.dirname(inspect.getsourcefile(foilselector)),
        "physicalparameters",
...
if __name__ == "__main__":
    ...
    assert os.path.exists(cl_arg.composition), (
        f"the -c, --composition argument ({cl_arg.composition}) must be a valid file path!"
    )
    assert all(os.path.exists(lib) for lib in cl_arg.library), (
        f"the -L, --library argument ({cl_arg.library}) must all be valid file paths!"
    )
    assert os.path.exists(cl_arg.photopeak_efficiency), (
        f"the -e, --photopeak-efficiency argument ({cl_arg.photopeak_efficiency}) must be a valid file path!"
    )
    ...
    xs_dict, decay_dict = sparsely_load_xs_and_decay_dict(
        _isotope_concerned, cl_arg.library
    )
    ...
    for name, dec_file in tqdm(
        ...
        gamma_energy_limits = (
            sorted(cl_arg.gamma_energy_limits_keV)
            if cl_arg.gamma_energy_limits_keV
    ...
    assert (gs_array[:, 0] < gs_array[:, 1]).all(), (
        "The group-structure file must be provided in ascending bin order! (And the flux file used in the next step must match it.)"
    )
