...
population = pd.read_csv(os.path.join(sys.argv[-1], ".counts.csv"), index_col=[0], comment="#")
...
full_sigma_matrix = pd.read_csv(os.path.join(sys.argv[-1], ".sigma_df.csv"), index_col=[0])
    ...
    for num_foils_chosen in num_foils_chosen_list:
        ...
        for method, args, appending_fname in zip((choose_top_n_pretty, perturb_top_1_order), (branching_args, perturbing_args), ("_specificity_by_branching.json", "_specificity_by_perturbing.json")):
            ...
            for key, val in descending_odict(top_specificity_dict).items():
                str_key = "-".join(key)
                foil_set = chain_sum([foil_candidates_dict[name] for name in key])
                saving_dict[str_key] = {"sensitivity-specificity":(get_sensitivity(foil_set.response_per_unit_flux, foil.counts.values), val), "min melting point":foil_set.get_min_melting_point()}
                if LIST_PRICE:
                    saving_dict[str_key]["price"] = foil_set.get_price_expr()
...
if EXAMPLE_UNFOLDINGS:=False:
    ...
    min_combo = choose_top_1( get_negative_specificity_from_unsorted_names, num_foils_chosen, list(foil_candidates_dict.keys()) )[0]
    ...
    if PERTURB_TOP_AND_BOTTOM_BEST_RANKED:=False:
        ...
        for combo in perturb_top_1_order( get_negative_specificity_from_unsorted_names, num_foils_chosen, list(foil_candidates_dict.keys()), skip_evaluation=True).keys():
            sample_foil_combos[combo] = chain_sum([foil_candidates_dict[foil] for foil in combo]).response_per_unit_flux
