...
APRIORI_FLUX, APRIORI_FLUENCE = get_apriori_from_folder(sys.argv[-1], float(input("What's the irradiation duration in seconds?")))
...
phy_prop_file_path = (
    "~/Self/foilselector/foilselector/physicalparameters/material_properties/elemental_frac_isotopic_frac_physical_short.csv"
)
phy_prop = get_physical_property(phy_prop_file_path)
if LIST_PRICE := False:
    from foilselector.physicalparameters.price import get_price_df, get_price_from_price_df, PRICE_FILE
        ...
        for method, args, appending_fname in zip(
            ...
            for key, val in descending_odict(top_specificity_dict).items():
                ...
                saving_dict[str_key] = {
                    "sensitivity-specificity": (
                        get_sensitivity(
                            foil_set.response_per_unit_flux,
                            foil.counts.values
                        ),
                        val,
                    ),
