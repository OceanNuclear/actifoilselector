from foilselector.script.step4.__init__ import *
...
_msg = "Output directory must already have gs.csv"
assert os.path.exists(os.path.join(sys.argv[-1], "gs.csv")), _msg
gs = pd.read_csv(os.path.join(sys.argv[-1], "gs.csv")).values
APRIORI_FLUX, APRIORI_FLUENCE = get_apriori_from_folder(
    sys.argv[-1], float(input("What's the irradiation duration in seconds?"))
)
...
_msg = "A '.sigma_df.csv' file must exist (generated by ReadData.py) at the target directory listed by the last argv argument."
assert os.path.exists(os.path.join(sys.argv[-1], ".sigma_df.csv")), _msg
tprint(
    "Reading the per reactant atom/ per primary product information from '.counts.csv'..."
)
population = pd.read_csv(
    os.path.join(sys.argv[-1], ".counts.csv"), index_col=[0], comment="#"
)
population = unserialize_pd_DataFrame(population)
# get the list of reactants for the reactions too
reactants = ary([
    parent_product_mt.split("-")[0] for parent_product_mt in population.index
])
tprint("Reading the microscopic cross-section...")
full_sigma_matrix = pd.read_csv(
    os.path.join(sys.argv[-1], ".sigma_df.csv"), index_col=[0]
)
...
phy_prop_file_path = "~/Self/foilselector/foilselector/physicalparameters/material_properties/elemental_frac_isotopic_frac_physical_short.csv"
phy_prop = get_physical_property(phy_prop_file_path)
if LIST_PRICE := False:
    from foilselector.physicalparameters.price import (
        get_price_df,
        get_price_from_price_df,
        PRICE_FILE,
    )
...
get_specificity_lin = lambda r: confusion_matrix_correctly_identified_diagonal(
    r, APRIORI_FLUENCE, confusion_matrix_linear
)
get_specificity_log = lambda r: confusion_matrix_correctly_identified_diagonal(
    r, APRIORI_FLUENCE, confusion_matrix_log
)
# There should be one more

tprint(
    "Stripping the uncertainties from the 'counts measured by detector per reactant atom' column..."
)
nominal_population = pd.DataFrame(
    nominal_values(population), index=population.index, columns=population.columns
)
...
for material_name, material in material_progress_bar:
    ...
    for species, species_density in partial_number_density[
        partial_number_density > 0
    ].iteritems():
    ...
    number_densities = pd.Series(
        number_densities, index=parent_product_mts, name="number density"
    )
    ...
    foil = Foil(
        ...
        population_slice[
            ["activity before measurement PPP", "activity after measurement PPP"]
        ],
...
for foil in foil_candidates:
    ...
    if LIST_PRICE:
        try:
            price = get_price_from_price_df(
                price_df, merged_foil.material_name, merged_foil.thickness
            )
...
individual_sensitivity = OrderedDict(
    (f_name, get_sensitivity(foil.response_per_unit_flux.values, foil.counts.values))
    for f_name, foil in foil_candidates_dict.items()
)
individual_specificity_cnt = OrderedDict(
    (f_name, get_specificity_cnt(foil.response_per_unit_flux.values))
    for f_name, foil in foil_candidates_dict.items()
)
individual_specificity_lin = OrderedDict(
    (f_name, get_specificity_lin(foil.response_per_unit_flux.values))
    for f_name, foil in foil_candidates_dict.items()
)
individual_specificity_log = OrderedDict(
    (f_name, get_specificity_log(foil.response_per_unit_flux.values))
    for f_name, foil in foil_candidates_dict.items()
)
num_reactions = OrderedDict(
    (f_name, foil.response_per_unit_flux.shape[0])
    for f_name, foil in foil_candidates_dict.items()
)
thicknesses = OrderedDict(
    (f_name, foil.thickness) for f_name, foil in foil_candidates_dict.items()
)
foil_data_df = pd.DataFrame(
    [
        thicknesses,
        num_reactions,
        individual_sensitivity,
        individual_specificity_cnt,
        individual_specificity_lin,
        individual_specificity_log,
    ],
    index=[
        "thickness",
        "number of reactions",
        "individual sensitivity",
        "individual specificity (count)",
        "individual specificity (linear)",
        "individual specificity (log)",
    ],
).T
...
while True:
    mask = ary([
        index not in excluded_materials.split(",") for index in foil_data_df.index
    ])
    ...
    if (
        "y"
        in input(
            "Are you satisfied with this selection of {} candidate materials? (y/n)".format(
                mask.sum()
            )
        ).lower()
    ):
    ...
    print(
        "If not, please enter the names of the foils that you would like to remove, delimited by ',':"
    )
...
saved_foil_params = OrderedDict(
    sorted(
        saved_foil_params.items(),
...
def get_negative_specificity_from_unsorted_names(*names):
...
if SCATTERPLOT := False:
    ...
    num_combinations = int(
        fac(len(foil_candidates_dict))
        / (fac(len(foil_candidates_dict) - num_foils_chosen) * fac(num_foils_chosen))
    )
    print(
        "Creating the scatter plot of sensitivity vs specificity, one data point for each of the {} choose {} = {} combination of foils:".format(
            len(foil_candidates_dict), num_foils_chosen, num_combinations
        )
    )
    for f_set in tqdm(
        itertools.combinations(foil_candidates_dict.values(), num_foils_chosen),
    ...
    plt.title(
        "{} foils choose {} = {} combinations possible ".format(
            len(foil_candidates_dict), num_foils_chosen, num_combinations
        )
    )
    plt.show()
...
if BUILD_SENSITIVITY_LIST := True:
    num_top_sensitivity = 5000
    print(
        "Trying to build the list of top sensitivity combinations, which consists of picking the top {} for {} different num_foils_chosen:".format(
            num_top_sensitivity, len(num_foils_chosen_list)
        )
    )
    for num_foils_chosen in tqdm(num_foils_chosen_list):
        top_sensitivity_combo = top_n_sums_of_dict(
            individual_sensitivity, num_foils_chosen, num_top_sensitivity
        )
        ...
        for key, val in descending_odict(top_sensitivity_combo).items():
            ...
            saving_dict[str_key] = {
                "sensitivity-specificity": (
                    val,
                    get_specificity_lin(foil_set.response_per_unit_flux),
                ),
                "min melting point": foil_set.get_min_melting_point(),
            }
            if LIST_PRICE:
        ...
        with open(
            os.path.join(
                sys.argv[-1], str(num_foils_chosen).zfill(2) + "_sensitivity_combo.json"
            ),
            "w",
        ) as j:
...
if BUILD_SPECIFICITY_LIST := True:
    ...
    for num_foils_chosen in num_foils_chosen_list:
        ...
        perturbing_args = (
            get_specificity_from_unsorted_names,
            num_foils_chosen,
            list(foil_candidates_dict.keys()),
            [0, 1, 2],
        )
        branching_args = (
            get_specificity_from_unsorted_names,
        ...
        for method, args, appending_fname in zip(
            (choose_top_n_pretty, perturb_top_1_order),
            (branching_args, perturbing_args),
            ("_specificity_by_branching.json", "_specificity_by_perturbing.json"),
        ):
            ...
            for key, val in descending_odict(top_specificity_dict).items():
                ...
                saving_dict[str_key] = {
                    "sensitivity-specificity": (
                        get_sensitivity(
                            foil_set.response_per_unit_flux, foil.counts.values
                        ),
                        val,
                    ),
                    "min melting point": foil_set.get_min_melting_point(),
            ...
            with open(
                os.path.join(
                    sys.argv[-1], str(num_foils_chosen).zfill(2) + appending_fname
                ),
                "w",
            ) as j:
                j.write(json.dumps(saving_dict, indent=4))
...
if EXAMPLE_UNFOLDINGS := False:
    ...
    min_combo = choose_top_1(
        get_negative_specificity_from_unsorted_names,
        num_foils_chosen,
        list(foil_candidates_dict.keys()),
    )[0]
    min_combo = tuple(sorted(min_combo))
    sample_foil_combos[min_combo] = chain_sum([
        foil_candidates_dict[foil] for foil in min_combo
    ]).response_per_unit_flux
    # add in guess max specificity
    max_combo = choose_top_1(
        get_specificity_from_unsorted_names,
        num_foils_chosen,
        list(foil_candidates_dict.keys()),
    )[0]
    max_combo = tuple(sorted(max_combo))
    sample_foil_combos[max_combo] = chain_sum([
        foil_candidates_dict[foil] for foil in max_combo
    ]).response_per_unit_flux

    if PERTURB_TOP_AND_BOTTOM_BEST_RANKED := False:
        for combo in perturb_top_1_order(
            get_specificity_from_unsorted_names,
            num_foils_chosen,
            list(foil_candidates_dict.keys()),
            skip_evaluation=True,
        ).keys():
            sample_foil_combos[combo] = chain_sum([
                foil_candidates_dict[foil] for foil in combo
            ]).response_per_unit_flux
    ...
    while len(sample_foil_combos) < num_foil_combo_samples:
        combo = np.random.choice(
            list(foil_candidates_dict.keys()), num_foils_chosen, False
        )
        sample_foil_combos[tuple(sorted(combo))] = chain_sum([
            foil_candidates_dict[foil] for foil in combo
        ]).response_per_unit_flux

    truth = apriori_fluence.copy()

    D_KL_result, chi2_result, ydev_result = (
        defaultdict(list),
        defaultdict(list),
        defaultdict(list),
    )
    ...
    false_aprioris = [
        false_apriori_generator(truth, i) for i in range(num_apriori_variations)
    ]
    starting_D_KL = [
        D_KL(secondary_apriori, truth) for secondary_apriori in false_aprioris
    ]
    print("Calcualting the sensitivity and specificity of all foils used.")
    foil_combo_coords = OrderedDict([
        (name, (get_sensitivity(fset_matrix), get_specificity_lin(fset_matrix)))
        for name, fset_matrix in tqdm(sample_foil_combos.items())
    ])
    ...
    for alg_name, alg_type in algorithms.items():
        print(
            ...
            alg_name,
            "calculating",
            num_samples := len(false_aprioris) * len(sample_foil_combos),
            "different cases:",
        )
        for secondary_apriori, (name, fset_matrix) in tqdm(
            itertools.product(false_aprioris, sample_foil_combos.items()),
            total=num_samples,
        ):
            ...
            alg_instance = alg_type(
                N_meas,
                matrix,
                secondary_apriori,
                sigma_N,
                desired_chi2=desired_chi2[alg_name],
                verbosity=0,
            )
        ...
        if not os.path.exists(os.path.join(sys.argv[-1], PLOT_SAVE_FOLDER)):
            os.mkdir(os.path.join(sys.argv[-1], PLOT_SAVE_FOLDER))
        ...
        for array, name in zip(
            (D_KL_wst, D_KL_avg), ((r"$D_{KL}$", "worst"), (r"$D_{KL}$", "average"))
        ):
            x, y, z = ary(array).T
            plt.suptitle("Calculated using the unfolding algorithm {}".format(alg_name))
            plt.title(
                " ".join(name[::-1])
                + " of {} different unfolding attempts".format(len(false_aprioris))
            )
            ...
            plt.text(
                *possibly_worst, "possibly worst specificity", va="top", ha="center"
            )
...
params_dict = dict(
    SATURATION_COUNT_RATE=SATURATION_COUNT_RATE,
    COUNT_THRESHOLD=COUNT_THRESHOLD,
    RANK_BY_DETERMINANT=RANK_BY_DETERMINANT,
    PLOT_SAVE_FOLDER=PLOT_SAVE_FOLDER,
    PHYSICAL_PROP_FILE=phy_prop_file_path,
)
if LIST_PRICE:
...
if PLOT_COMPLETE_CURVATURE := False:
    try:
        ...
        while True:
            ...
            plt.title(
                f"Confusion matrix of the foilset with {len(f_set.material_name)} foils"
            )
            plt.show()
            ...
            f_set = FoilSet(*[
                foil
                for f_name, foil in foil_candidates_dict.items()
                if f_name not in ignored_foils
            ])
...
if RANK_BY_DETERMINANT:
    raise NotImplementedError(
        "This feature, while easy to implement, is unlikely to be useful (only useful in very overdetermined case) so it hasn't been implemented yet!"
    )
